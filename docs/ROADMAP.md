| Task                                            | Slated for | Actual release | Comments                             |
|-------------------------------------------------|------------|----------------|--------------------------------------|
| [x] auth                                        | 0.10.00    | same           |                                      | 
| [x] login                                       | 0.10.00    | same           |                                      |
| [x] remote host connection                      | 0.10.00    | same           |                                      |
| [x] http(s) thin client                         | 0.10.00    | same           |                                      |
| [x] tab completion                              | 0.10.00    | same           |                                      |
| [x] image list                                  | 0.40.00    | same           |                                      |
| [x] image pull                                  | 0.10.00    | same           |                                      |
| [x] image push                                  | 0.20.00    | same           |                                      |
| [x] container list                              | 0.20.00    | 0.21.00        |                                      |
| [x] container info                              | 0.20.00    | 0.30.00        |                                      |
| [x] container stop/start                        | 0.30.00    | 0.30.00        |                                      |
| [x] container pause/unpause                     | 0.30.00    | 0.30.00        |                                      |
| [x] container rm                                | 0.20.00    | 0.50.00        |                                      |
| [x] container rename                            | 0.40.00    | 0.50.00        |                                      |
| [x] container restart                           | 0.55.00    | same           | that function had been forgotten !   |
| [x] image rm                                    | 0.40.00    | 0.51.00        |                                      |
| [x] image tag                                   | 0.40.00    | 0.50.00        |                                      |
| [ ] network create / rm                         |            |                |                                      |
| [ ] network attach / detach                     |            |                |                                      |
| [ ] network / volume ls                         |            |                |                                      |
| [ ] volume create / rm                          | 0.20.00    |                |                                      |
| [ ] exec (shell)                                |            |                |                                      |
| [ ] system info                                 |            |                |                                      |
| [x] blacklist list [-a]                         | 0.20.00    | 0.30.00        | fixed regression in 0.50.00          |
| [x] blacklist rm                                | 0.20.00    | 0.30.00        | fixed regression in 0.50.00          |
| [x] blacklist add                               | 0.20.00    | 0.30.00        | fixed regression in 0.50.00          |
| [x] container kill / killall                    | 0.50.00    |                |                                      | 
| [ ] `dockerrm.sh`                               | 0.60.00    |                | porting my bash script to dtools     |
| [ ] `dockerclean.sh`                            | 0.60.00    |                | porting my bash script to dtools     |
| [ ] rebranding `dtools2`<br>as `dtools 2.00.00` | 2.00.00    |                |                                      |