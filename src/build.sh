...
DTVERSION="$(sed -n 's/.*"versionnumber": *"\([^"]*\)".*/\1/p' "$SCRIPT_DIR/../dtools2.json")"
BUILDDATE="$(date +%Y.%m.%d)"

CGO_ENABLED=0 go build -trimpath -ldflags="-s -w -buildid= -X dtools2/cmd.buildVersion=$DTVERSION -X dtools2/cmd.buildDate=$BUILDDATE" -o "$BUILD_OUTPATH"   .
