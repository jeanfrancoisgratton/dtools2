#!/usr/bin/env sh
# dtools2 — bump the software version everywhere it needs to be typed by
...
TARGETS="dtools2.json __alpine/APKBUILD __archlinux/PKGBUILD __debian/control __redhat/dtools2.spec"

print_usage() {
  cat <<EOF
Usage: $0 <new-version>   (e.g. $0 1.3.2)

Bumps the software version in dtools2.json and every __*/ packaging file,
...
echo "Bumping dtools to version $NEWVERSION (release reset to 1 on every distro)"

patch dtools2.json \
  "s/\"versionnumber\": *\"[^\"]*\"/\"versionnumber\": \"$NEWVERSION\"/
   s/\"defaultreleasenumber\": *\"[^\"]*\"/\"defaultreleasenumber\": \"1\"/"
verify dtools2.json "\"versionnumber\": \"$NEWVERSION\""
verify dtools2.json "\"defaultreleasenumber\": \"1\""
...
patch __redhat/dtools2.spec \
  "s/^%define _version .*/%define _version $NEWVERSION/
   s/^%define _rel .*/%define _rel 1/"
verify __redhat/dtools2.spec "%define _version $NEWVERSION"
verify __redhat/dtools2.spec "%define _rel 1"
...
echo
echo "Changed:"
grep -H '"versionnumber"\|"defaultreleasenumber"' dtools2.json
grep -H '^pkgver=\|^pkgrel=' __alpine/APKBUILD
grep -H '^pkgver=\|^pkgrel=' __archlinux/PKGBUILD
grep -H '^Version:' __debian/control
grep -H '^%define _version\|^%define _rel' __redhat/dtools2.spec
echo
