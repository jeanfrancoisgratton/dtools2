...
func (rb *ResourceBlacklist) List(resourceType string) ([]string, error) {
...
func ListFromFile(resourceType string) error {
	...
	if rsources, rbErr = rb.List(resourceType); err != nil || len(rsources) == 0 {
		return rbErr
	} else {
		rmp = map[string][]string{
			resourceType: append([]string(nil), rsources...)}
	}
	return outputBLlist(rmp)
}
